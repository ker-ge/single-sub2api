# Sub2API 0.1.150（Linux amd64 / ARM64）
...
OpenAI 模型目录新增 `gpt-6-sol` 和 `gpt-6-luna`，并保留 `gpt-6-astra`；三种模型均支持统一的 GPT-6 模型映射、推理档位和官方价格配置。
...
两个程序内版本均为 0.1.150。将两个成品及脚本一并提交并 push，在同一提交上创建并推送新标签 v0.1.150，不移动旧标签。已安装新版多架构 helper 的服务器可在版本弹窗确认更新。旧标签若只有 amd64 文件，ARM64 在线更新会安全拒绝。尚未替你 push、打标签或部署服务器。
